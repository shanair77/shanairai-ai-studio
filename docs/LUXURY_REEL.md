...
| Build | 13–22 s | sixteen new locations at half a beat — faster, and nothing the viewer has seen yet |
| Land | 22–27 s | four fresh shots slowing back down, then one callback to the hero shot |
...
1. **Footage.** The thirty-six shots are Higgsfield-generated clips (Kling 3.0 pro, 9:16,
   5 s) stored in `public/luxury/` and listed in `src/luxury/shots.ts`. 38 cuts draw on 36
   distinct shots, so only the hero and end-card shots ever repeat. To re-cast
...
- **Series:** keep the cut list, swap the `src` values per city — Paris, Dubai, Monaco.
