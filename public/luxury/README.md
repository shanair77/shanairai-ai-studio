...
The first sixteen shots were generated with Higgsfield (Kling 3.0 pro, 9:16, 5 s, silent; `chef`,
`champagne`, `aerial` and `watch` with Seedance 2.5 at 1080p) and
The twenty added in September 2026 (`porsche-street` onward) are Kling 3.0 pro as well.
`src/luxury/shots.ts` points at the local files here. If a shot is ever re-pointed at a CDN URL,
make it permanent again with:
...
| `porsche-street` | Vintage Porsche on wet cobblestones under a streetlamp | amber |
| `infinity-pool` | Rooftop infinity pool reflecting the skyline | cool blue |
| `whisky` | Whisky poured over an ice sphere, bar light | amber |
| `cufflinks` | Cufflinks and a watch on a velvet tray | silver |
| `yacht-deck` | Superyacht teak deck at dusk | pink / purple |
| `helicopter` | Helicopter on a rooftop helipad, city below | deep blue |
| `spiral-stairs` | Marble spiral staircase, brass rail | cream |
| `wine-cellar` | Stone wine cellar by candlelight | amber |
| `piano` | Grand piano in a dim lounge | gold |
| `bentley-grille` | Bentley grille and headlights in the rain | silver |
| `rooftop-bar` | Rooftop terrace bar with string lights | gold |
| `dessert` | Plated dessert with gold leaf | warm white |
| `jet-cabin` | Private jet cabin, cream leather | cream |
| `dubai` | Dubai skyline from a terrace | deep blue |
| `monaco` | Monaco harbour at night | gold |
| `perfume` | Backlit perfume bottle on marble | champagne |
| `elevator` | Gold-mirrored elevator, doors opening | gold |
| `fireplace` | Fireplace with leather armchairs | amber |
| `villa-pool` | Villa pool glowing violet | violet |
| `valet-keys` | Valet handing over keys, hands only | amber |
...
- **At least 1.5 s per clip.** The longest single cut is 3.3 s (`penthouse` end-card); every
  other shot is used for ≤ 1.4 s, and only `lambo-arch` and `penthouse` appear twice. Use `trimStart` on a shot to pick the best moment.
